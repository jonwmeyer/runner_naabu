...
def main():
    """Entry point: Validate input, check dependencies, run naabu, and handle results."""
    if len(sys.argv) < 2:
        print("[!] Error: Please provide a host for naabu")
    ...
    print(f"[*] Starting naabu query for: {host}")
    exit_code = run_naabu_and_save(host)
    
    if exit_code == 0:
        print("[+] naabu completed successfully")
    else:
        print("[!] naabu completed with errors or warnings")
...
def run_naabu_and_save(host):
    """Run naabu and save results to a timestamped file."""
    try:
        output = run_naabu(host)
        if output is None:
